...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existingMockApi) {
      return NextResponse.json(
        { error: "이미 존재하는 Mock API입니다." },
        { status: 409 }
      );
    }

    // JSON 데이터 정규화 (이중 인코딩 방지)
    const normalizeJsonData = (data: unknown): Prisma.JsonValue => {
    ...
    const mockApi = await prisma.proxyMockApi.create({
      data: {
        ...
        method: method.toUpperCase(),
        apiName: `${method} ${path}`,
        requestSpec: normalizedRequestBody ? { schema: normalizedRequestBody } : undefined,
        responseSpec: normalizedResponseBody ? { schema: normalizedResponseBody } : undefined,
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Mock API가 성공적으로 생성되었습니다.",
        mockApi,
      },
      { status: 201 }
    );
